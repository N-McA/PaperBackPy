...
class QRCodeBuilder:
    ...
    * http://www.matchadesign.com/blog/qr-code-demystified-part-6/
    ...
    def add_words(self):
        ...
        return ''.join([next(block) for x in xrange(needed_blocks)])
    ...
    def make_error_block(self, block, block_number):
        ...
        * http://www.matchadesign.com/blog/qr-code-demystified-part-4/
        ...
        for i in xrange(code_words_per_block):
    ...
    def make_code(self):
        ...
        row = [' ' for x in xrange(matrix_size)]
        template = [deepcopy(row) for x in xrange(matrix_size)]
    ...
    def make_masks(self, template):
        ...
        for n in xrange(nmasks):
            ...
            for column in xrange(len(cur_mask)-1, 0, -2):
                ...
                for row in xrange(next(row_start), next(row_stop),
                                 next(direction)):
    ...
    def choose_best_mask(self):
        ...
        """
        self.scores = []
        for n in xrange(len(self.masks)):
        ...
        for i in xrange(len(self.scores)):
            for j in xrange(len(self.scores[i])):
